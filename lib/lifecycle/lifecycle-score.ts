...
export function calculateLifecycleScore(input: SkuLifecycleInput): SkuLifecycleScore {
  const age = input.product_age_days;
  const growth = input.revenue_growth ?? 0;
  const roas = input.roas ?? null;
  const confidence = input.prediction_confidence ?? 0.55;
  ...
  const hasStableHistory = (age === undefined || age >= 45) && orderCount >= 30 && confidence >= 0.55;
  const hasAdsHistory = input.ads_spend > 0 && roas !== null && Number.isFinite(roas);
  const inventoryAvailable = input.inventory > Math.max(10, input.sales_velocity * 14);
  const inventoryAccumulating = inventoryRunway !== null && inventoryRunway > 90;

  const launch = clamp01(
    (age !== undefined && age < 30 ? 0.42 : 0) +
      (orderCount < 30 ? 0.24 : 0) +
      (!hasAdsHistory ? 0.16 : 0) +
      (confidence < 0.55 ? 0.18 : 0)
  );
  const growthScore = clamp01(
    (growth > 0.12 ? 0.32 : growth > 0.03 ? 0.18 : 0) +
      (profit > 0 ? 0.2 : 0) +
      (roas !== null && roas >= 3 ? 0.22 : roas !== null && roas >= 2 ? 0.12 : 0) +
      (inventoryAvailable ? 0.16 : 0) +
      (confidence >= 0.65 ? 0.1 : 0)
  );
  const mature = clamp01(
    (hasStableHistory ? 0.24 : 0) +
      (Math.abs(growth) <= 0.08 ? 0.24 : 0) +
      (profit > 0 ? 0.18 : 0) +
      (input.margin >= 0.18 ? 0.16 : 0) +
      (input.repeat_rate !== undefined && input.repeat_rate >= 0.12 ? 0.1 : 0.04) +
      (inventoryRunway !== null && inventoryRunway >= 21 && inventoryRunway <= 90 ? 0.08 : 0)
  );
  const decline = clamp01(
    (growth < -0.08 ? 0.3 : growth < -0.02 ? 0.14 : 0) +
      (profit < 0 ? 0.22 : 0) +
      (roas !== null && roas < 1.5 && input.ads_spend > 0 ? 0.2 : 0) +
