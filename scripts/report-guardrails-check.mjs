...
assert.match(dashboard, /type ReportTimeRange = "TODAY" \| "7D" \| "30D" \| "90D" \| "12M" \| "ALL" \| "CUSTOM"/, "Report page should support real time range selection");
assert.match(dashboard, /\{ value: "TODAY", label: "Today" \}/, "Report page should expose Today as the first quick range");
assert.doesNotMatch(dashboard, /reportTimeRangeOptions[\s\S]*\{ value: "90D"/, "Report page should not show the removed 90D quick range");
...
const reportDateRange = read("lib/report-date-range.ts");
const reportTrendGuardrails = read("lib/report-trend-guardrails.mjs");
...
assert.match(reportDateRange, /order_date[\s\S]*transaction_date[\s\S]*event_time[\s\S]*review_date/, "Report date range resolver should detect business time fields");
...
assert.match(reportTrendGuardrails, /categorical_dimension/, "Trend guardrail should classify categorical dimensions");
assert.match(reportTrendGuardrails, /canUseAsTrendMetric/, "Trend guardrail should expose trend metric validation");
assert.match(reportTrendGuardrails, /sales_channel[\s\S]*category[\s\S]*segment[\s\S]*status/, "Trend guardrail should block common dimension fields as trend metrics");
assert.match(dashboard, /chartGroupLabel/, "Dashboard charts should be grouped by business purpose");
assert.match(dashboard, /核心趋势[\s\S]*结构分析[\s\S]*风险与机会/, "Dashboard should expose core trend, structure, and risk/opportunity chart groups");
assert.match(dashboard, /trendInsight/, "Dashboard trend charts should generate data-specific insight hints");
assert.match(dashboard, /rankingInsight/, "Dashboard ranking charts should generate data-specific insight hints");
assert.match(dashboard, /未完整周期/, "Dashboard trend charts should flag incomplete periods");
assert.match(dashboard, /aggregationType/, "Dashboard chart tooltips should expose aggregation type");
assert.match(dashboard, /chartMetricText/, "Dashboard chart titles should use business metric labels instead of raw field names");
assert.match(dashboard, /function dedupeCharts/, "Chart recommender should dedupe repeated charts before display");
assert.match(dashboard, /linkedInsightIds/, "Chart configs should link back to supporting insights");
assert.match(dashboard, /linkedMetricIds/, "Chart configs should link back to supporting metrics");
assert.match(dashboard, /businessQuestion/, "Chart configs should state the business question they answer");
assert.match(dashboard, /visibleCharts = sortedCharts\.slice\(0, 5\)/, "Recommended charts should show at most five charts by default");
assert.match(dashboard, /hiddenCharts = sortedCharts\.slice\(5\)/, "Additional charts should be folded into the more charts section");
...
const dashboardReportRoute = read("app/api/dashboard/reports/route.ts");
const dataSourceUploadRoute = read("app/api/data-sources/upload/route.ts");
const dataSourceUploadCompleteRoute = read("app/api/data-sources/upload/complete/route.ts");
const nextConfig = read("next.config.ts");
const uploadLimits = read("lib/upload-limits.ts");
const uploadsPresignRoute = read("app/api/uploads/presign/route.ts");
const reportTimeArtifacts = read("lib/report-time-artifacts.mjs");
const prismaSchema = read("prisma/schema.prisma");
const reportComposers = read("lib/report-composers.ts");
const reportComposersTest = read("tests/report-composers.test.mjs");
assert.match(reportTypes, /dateField\?: string/, "TimeTrendResult should preserve the business date field used for trends");
assert.match(reportGenerateRoute, /@\/lib\/report-time-artifacts\.mjs/, "Async report generation must use the shared backend time artifact builder");
assert.match(dashboardReportRoute, /@\/lib\/report-time-artifacts\.mjs/, "Ranged report API must use the shared backend time artifact builder");
assert.match(reportTimeArtifacts, /function dedupeTrendCandidates/, "Backend time artifacts must dedupe repeated trend candidates before payload persistence");
assert.match(reportTimeArtifacts, /canonicalTrendMetricKey/, "Backend time artifacts must normalize equivalent metric names before dedupe");
assert.match(reportTimeArtifacts, /hasValidBusinessRange/, "Backend time artifacts must reject impossible business metric ranges such as rating > 5");
assert.match(reportTimeArtifacts, /trendInsight/, "Backend time artifacts must generate chart-specific insight hints instead of generic copy");
assert.doesNotMatch(reportTimeArtifacts, /用于识别增长、下滑和波动/, "Backend trend chart payload must not use generic trend insight copy");
...
assert.match(prismaSchema, /model MetricSnapshot/, "Prisma schema must persist MetricSnapshot history for report comparisons");
assert.match(prismaSchema, /model ReportHistory/, "Prisma schema must persist generated Daily/Weekly/Custom report history");
assert.match(reportComposers, /export type ReportMode[\s\S]*daily_brief[\s\S]*weekly_report[\s\S]*snapshot_report/, "Report composers must expose reportMode variants");
assert.match(reportComposers, /composeDailyBrief/, "Daily Brief must be composed from existing analysis results");
assert.match(reportComposers, /composeWeeklyReport/, "Weekly Report must be composed from existing analysis results");
assert.match(reportComposers, /saveMetricSnapshots/, "MetricSnapshot must be saved after metric calculation");
assert.match(reportGenerateRoute, /normalizeReportMode\(payloadRecord\.reportMode\)/, "Report generation API must accept reportMode");
assert.match(reportGenerateRoute, /saveMetricSnapshots/, "Report generation must save MetricSnapshot records");
assert.match(reportGenerateRoute, /reportHistoryModel[\s\S]*create/, "Report generation must save ReportHistory records");
assert.match(dashboardReportRoute, /reportHistory/, "Report loading API must return report history");
assert.match(dashboard, /日报[\s\S]*周报[\s\S]*自定义报告[\s\S]*历史记录/, "Reports page must expose Daily, Weekly, Custom, and History tabs in Chinese");
...
assert.match(dashboard, /businessOpportunityCopy/, "Growth opportunity cards should rewrite technical terms into business language in Chinese mode");
assert.match(dashboard, /评分表现排在前 25%/, "Growth opportunity cards should translate rating percentile evidence");
assert.match(dashboard, /当前样本量还不大/, "Growth opportunity cards should translate raw record-volume caveats");
assert.match(dashboard, /技术口径：/, "Growth opportunity technical criteria should be kept behind the definition/details control");
assert.match(reportComposers, /function businessOpportunityText/, "Backend report composers should sanitize growth opportunity copy before persistence");
assert.match(reportComposers, /kind === "opportunity" \? businessOpportunityText/, "Backend opportunity cards should use business copy for visible fields");
assert.match(reportComposersTest, /weekly growth opportunities use business copy instead of technical terms/, "Backend report tests should lock opportunity copy against technical terms");
assert.match(aggregationEngine, /评分高于大多数对象/, "Generated growth opportunities should use business-readable comparison text");
assert.doesNotMatch(aggregationEngine, /comparison: `[^`]*(高于 P75|中位数或 P25|低于或接近中位数|AverageRating|records 不高)[^`]*`/, "Growth opportunity main comparisons should not expose technical terms");
assert.match(uploadLimits, /FILE_UPLOAD_MAX_BYTES\s*=\s*100 \* 1024 \* 1024/, "Upload size limit should be centralized at 100MB");
assert.match(nextConfig, /middlewareClientMaxBodySize:\s*FILE_UPLOAD_MAX_BYTES/, "Next.js should use the shared 100MB upload request body limit");
assert.match(dataSourceUploadRoute, /FILE_UPLOAD_MAX_BYTES/, "Backend direct upload route should use the shared upload limit");
assert.match(uploadsPresignRoute, /FILE_UPLOAD_MAX_BYTES/, "Backend presign upload route should use the shared upload limit");
assert.match(dataSourceUploadCompleteRoute, /FILE_UPLOAD_MAX_BYTES/, "Backend upload completion route should use the shared upload limit");
assert.match(dashboard, /directApiUploadMaxBytes\s*=\s*FILE_UPLOAD_MAX_BYTES/, "Frontend should use the shared upload limit for direct backend uploads");
